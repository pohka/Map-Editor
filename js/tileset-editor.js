...
class TilesetEditor extends Viewport{
  ...
  addInput(){
    ...
    vp.canvas.addEventListener("mousedown", function(e){
      ...
      if(tileID > -1){
        ...
        tile.collision = Store.selected.collisionType;
      }
  ...
  getTileIDsOfSelectedTileset(){
    ...
    for(let i in Store.tiles){
      if(Store.tiles[i].src == Store.selected.palette){
        ids.push(Store.tiles[i].id);
      }
    }
    return ids;
  }
